...
if __name__ == '__main__':
    ...
    # ========== 修改测试：反复插入相同值50次（已完成，注释保留） ==========
    # for i in range(50):
    #     Insert('apple')
    #     print(f"Insert #{i+1} done, encoding from DB:")
    #     conn = pymysql.connect(host='localhost', user='user', passwd='123456', database='test_db')
    #     cur = conn.cursor()
    #     cur.execute("SELECT encoding FROM example ORDER BY encoding DESC LIMIT 1")
    #     result = cur.fetchone()
    #     if result:
    #         print(f"  Latest encoding: {result[0]}")
    #     conn.close()
    # Search('a', 'b')
