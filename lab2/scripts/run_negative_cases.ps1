...
$ScriptWsl = "$RepoRootWsl/scripts/wsl_run_negative_cases.sh"

Write-Host "Running negative cases in WSL workspace: $WorkspaceRoot"
& wsl -d $Distro -e sh $ScriptWsl $RepoRootWsl $WorkspaceRoot

if ($LASTEXITCODE -ne 0) {
    throw "Lab2 negative-case run failed."
}
